...
try:
    ...
    print ("getopterrror")
    print ('usage is : \n python get_pseudoalignments_paired_end.py -i input-read-dir -o output-tcc-dir -k path-to-hacked-kallisto -t path-to-human-reference-transcriptome [-n number-of-processes-to-use]')
    sys.exit(1)
...
def get_pseudoalignments(fltuple):
    ...
    read_path_1=read_dir+flname+'_1.fastq.gz'
    read_path_2=read_dir+flname+'_2.fastq.gz'

    #print(flname)
    command = code_path+' pseudoalign -i '+ ref_path+ ' -o ' + out_path + ' ' + read_path_1+ ' '+read_path_2
    #print(command)
    os.system(command)
...
relevant_filenames=[files for files in flnames if files.endswith('_1.fastq.gz')]
#print relevant_filenames
file_names=sorted([x.split('_')[0] for x in relevant_filenames])
