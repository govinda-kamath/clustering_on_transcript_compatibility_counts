...
bool CheckOptionsPseudo(ProgramOptions& opt, bool emonly = false) {
  ...
  if (opt.single_end && (opt.fld == 0.0 || opt.sd == 0.0)) {
    cerr << "[pseudo] fragment length mean and sd were not supplied.. its ok! We only need them for quant" << endl;
    //ret = false;
  ...
  {
  std::string path;   
  size_t i = opt.output.rfind('/', opt.output.length());
   if (i != string::npos) {
      path=opt.output.substr(0, i);
      cerr<< path;
   }else{cerr << "Error: need to specify valid output directory and filename " << opt.output << endl;
    ret = false;}
   
    struct stat stFileInfo;
    auto intStat = stat(path.c_str(), &stFileInfo);
    if (intStat == 0) {
      // file/dir exits
      if (!S_ISDIR(stFileInfo.st_mode)) {
        cerr << "Error: file " << path << " exists and is not a directory" << endl;
        ret = false;
      } else if (emonly) {
...
void usagePseudoalign() {
  cout << "kallisto " << KALLISTO_VERSION << endl
       << "Computes equivalence classes for reads and outputs the corresponding counts" << endl << endl;
  
  cout << "Usage: kallisto pseudoalign [arguments] FASTQ-file" << endl << endl
       << "Required arguments:" << endl
       << "-i, --index=STRING            Filename for the kallisto index to be used" << endl
       << "-o, --output-dir=STRING       path/filename to write output to (plaintext)" << endl << endl;
  ...
  if (argc < 2) {
    ...
    if (cmd == "version") {
      ...
      if (!CheckOptionsInspect(opt)) {
        ...
        std::string outfile_2 = opt.index +"_tr_id_names.txt";
        std::ofstream of2;
        of2.open(outfile_2.c_str(), std::ios::out); 
        for (int tid=0; tid<index.target_names_.size(); tid++){
            of2 << tid << "\t" << index.target_names_[tid] << "\n";
        }
        of2.close();
        
        
        std::cout << "[metadata] output transcript names in "<< opt.index + "_tr_id_names.txt "<<endl;
        
        
        std::string outfile_3 = opt.index +"_ecmap.txt";
        std::ofstream of3;
        ...
        std::cout << "[metadata] output equiv.class map in "<< opt.index + "_ecmap.txt "<<endl;
